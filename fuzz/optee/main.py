...
from .opteedata import TeeIoctlInvokeArg
...
def setup_args():
    ...
    deserialize_parser.add_argument("raw_invoke_arg_path")
    serialize_parser.add_argument("pickled_invoke_arg_path")
...
def main():
    ...
    if "serialize" == args.subparser:
        # handle serialize
        raw_data_dict = TeeIoctlInvokeArg \
                        .serialize_raw_from_path(args.pickled_invoke_arg_path)
        if args._out:
        ...
        # handle deserialize
        ctx = TeeIoctlInvokeArg.deserialize_raw_from_path(
            args.raw_invoke_arg_path)
        if args._out:
            # save to dir if `_out` is set, print to stdout otherwise
            if os.path.isdir(args._out):
                filepath = os.path.join(args._out,
                                        "TC_NS_ClientContext.pickle")
